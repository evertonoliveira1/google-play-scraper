...
const scriptData = require('./../../src/utils/scriptData');
const appList = require('./../../src/utils/appList');
...
const checkFinished = (opts, savedApps, nextToken, fSid) => {
  ...
  return request(requestOptions, opts.throttle)
    .then((html) => {
      ...
      const htmlData = html.substring(initialIndex, finalIndex);      
      const input = JSON.parse(htmlData);

      const data = JSON.parse(input[0][2]);

      return processAndRecur(data, opts, savedApps, REQUEST_MAPPINGS, fSid, reqId);
    });
}
...
const initialRequest  = (opts)  => {
  ...
  console.log('initial url', builtUrl);
